...
### BTC penalizza se stesso contro il proprio trend (brain_la.py)
**Problema:** La penalità BTC corr saltava BTC stesso (is_btc=True). BTC LONG 16:06 aperto con H1 DOWNTREND confermato → perdita -1.03$ non protetta.
**Correzione:** Aggiunto blocco elif per BTC: penalità max -2 se struttura H1 è opposta al segnale E Hurst > 0.55. Non usa correlazione esterna ma i propri dati Engine.
**Esempio:** BTC LONG con H1 DOWNTREND + Hurst 0.57 → penalità -2 → voto 7→5, BLOCCATO.

### Chiusura parziale strategica troppo precoce (trade_manager.py)
**Problema:** La chiusura parziale scattava a +1% fisso di profitto con pressione muro > 80%. Su BTC SHORT con TP al 2.3%, a +1% dimezzava la size, poi il trailing stop chiudeva l'altra metà a +1.1% invece del TP a +2.6%.
**Correzione:** La chiusura parziale ora scatta solo quando si è percorso ≥70% del cammino verso il TP + pressione muro >80%. A +1% su TP 2.3% = 43% avanzamento → NON scatta. A +1.6% (70% del cammino) → scatta se muro blocca.
**Impatto simulato BTC SHORT 13:18:** Con TP 2.3%, scattava a +1% (43%), ora scatterebbe a +1.6%. Profitto +2.6$ invece di +1.08$.

### Penalità OFI + Whale Delta — sistema di crediti graduato (brain_la.py)
**Filosofia:** Nessun veto assoluto — solo penalità sul voto. Un segnale con flusso contrario non viene bloccato se ha altri elementi forti, ma riceve meno credito. Più indicatori contro = più penalità.

**Problema risolto:** Gemini ignorava sistematicamente OFI e whale_delta (3 trade su 16 il 24/03). XRP LONG 08:19 aveva whale_delta -1.0 e OFI -0.654 ma Gemini ha scritto "order flow positivo" — razionale inventato.

**Correzioni al codice:**
1. `whale_delta` e `delta_footprint` aggiunti all'estrazione variabili (non erano estratti)
2. Entrambi aggiunti al JSON passato a Gemini con note esplicative inline
3. Guida interpretativa (punti 7 e 8 aggiunti) spiega come usarli
4. Penalità automatica post-Gemini come rete di sicurezza:
   - OFI forte contro + Whale forte contro → -3
   - OFI + Whale contro (moderati) → -2
   - Solo OFI forte contro (e CVD non compensa) → -1
   - Solo Whale forte contro → -1
   - **Condizione CVD:** se CVD concorda con la direzione, la penalità OFI si annulla (OFI può essere muro temporaneo che viene assorbito)

**Simulazione 24/03 — zero falsi positivi:**
- XRP LONG 08:19: OFI -0.65 + Whale -1.0 → -3 → voto 4 🚫 (perdita evitata)
- BTC SHORT 13:19: OFI +0.93 contro ma CVD -152k compensa → 0 penalità ✅
- BTC LONG 07:04: tutto allineato → 0 penalità ✅
- Cap penalità totale alzato da -4 a -5 per includere il nuovo layer

### Multi-timeframe integrato in Gemini — senza nuove API (brain_la.py)
**Problema:** Gemini riceveva solo lo snapshot 15m. Apriva LONG con 4h e 1d in DOWNTREND perché non aveva visione della struttura più grande.

**Approccio:** Nessuna chiamata API nuova — i dati multi-tf (15m, 1h, 4h, 1d) erano già calcolati dall'Engine ma venivano scartati dopo l'Hurst.

**Tre modifiche:**

1. **Dati multi-tf espansi al JSON Gemini** — ora passa trend_dir, regime, vol_relativo, ema20_vs_ema50 per ogni timeframe (prima solo Hurst)

2. **Narrativa strutturale testuale** — testo leggibile prima del prompt: "1d: 📉 DOWN | trend | Hurst 0.56 / 4h: 📉 DOWN | trend..." + istruzione esplicita: "la struttura grande definisce il BIAS, il 15m dà il TIMING"

3. **Penalità automatica confluenza multi-tf** — rete di sicurezza se Gemini ignora:
   - 4h contro → -1
   - 1d contro → -1
   - 4h + 1d contro → -2
   - + 1h contro (se c'è già penalità) → -1 aggiuntivo
   - Si attiva SOLO su regime TRENDING + Hurst > 0.52 (in range è normale andare contro il TF superiore)

**Simulazione 24/03 — zero falsi positivi:**
- ETH LONG 00:07: 4h+1d DOWN → -2 → voto 5 🚫
- XRP LONG 12:47: 4h+1d DOWN → -2 → voto 5 🚫 (+ -3 flow = voto 2)
- SOL LONG 13:22: 4h+1d DOWN → -2 → voto 5 🚫
- BTC LONG 16:06: 4h+1d DOWN → -2 → voto 5 🚫
- BTC LONG 07:04: 4h+1d UP → 0 ✅
- BTC SHORT 13:19: SHORT con 4h+1d DOWN → 0 ✅ (allineato)

### Pattern candlestick + bonus confluenza positiva (engine_la.py + brain_la.py)
**Problema 1:** Nessuna analisi candlestick nel sistema.
**Problema 2:** Il sistema solo penalizzava — nessun premio per setup perfetti.

**Engine — nuova funzione pattern candlestick:**
Calcolata su ultime 3 candele 15m. Pattern rilevati: DOJI, HAMMER, HANGING_MAN, SHOOTING_STAR, PIN_BAR (bull/bear), MARUBOZU, ENGULFING (bull/bear), INSIDE_BAR, OUTSIDE_BAR, MORNING_STAR, EVENING_STAR.
Output: `candlestick_patterns` (lista), `candlestick_bias` (BULLISH/BEARISH/INDECISO/NEUTRO).

**Brain — bonus confluenza positiva (max +2, dopo penalità):**
- Trend 4h+1h+1d tutti allineati: +2
- Trend 4h+1h allineati: +1
- Solo 4h allineato: +1
- Candela forte nella direzione (ENGULFING, MARUBOZU, PIN_BAR, MORNING/EVENING_STAR): +1
- OFI > 0.6 e Whale > 0.6 entrambi allineati: +1
Cap: voto massimo 10.

**Simulazione 24/03:**
- BTC LONG 07:04: 4h+1h UP + ENGULFING_BULL → voto 7+2 = 9 🔥
- BTC SHORT 13:19: 4h+1h+1d DOWN + MARUBOZU_BEAR → voto 7+2 = 9 🔥
- XRP LONG 08:19: pen -4 + bonus 0 → voto 3 🚫
- ETH LONG 00:07: pen -2 + bonus 0 → voto 5 🚫

### Mappa contestuale gerarchia livelli (brain_la.py)
**Problema:** Un muro book del 15m veniva trattato con lo stesso peso di un livello strutturale H4 o weekly. Il bot calcolava TP a 1.53 su XRP mentre tra il prezzo e quel TP c'erano resistenza strutturale H1, R2 daily, POC weekly — ostacoli reali che il bot non vedeva.

**Soluzione:** Prima del prompt Gemini viene costruita una mappa contestuale che risponde a "dove siamo nella struttura?":
- Posizione nella Value Area settimanale (dentro/sopra/sotto VA)
- Livelli strutturali vicini entro 1.5% (pivot weekly/daily, R1/R2/S1/S2, swing strutturali)
- Ogni muro book confrontato con i livelli superiori: se coincide con H4 swing, VAH/VAL, pivot → "LIVELLO FORTE", altrimenti "LIVELLO DEBOLE, attraversabile"
- Regola esplicita a Gemini: valuta il TP in base a quanti livelli forti ci sono tra prezzo e target

**Dati già disponibili — zero nuove API:**
POC weekly, VAH/VAL, pivot daily/weekly con R1/R2/S1/S2, swing high/low H4, resistenza/supporto strutturale H1 — tutti già calcolati, mai messi in relazione tra loro.

### Memoria strutturale — salvataggio e valutazione retrospettiva (brain_la.py + trade_manager.py)
**Problema:** La lettura strutturale (dove eravamo nella VA, quali livelli erano vicini, se il muro era confermato) veniva costruita, passata a Gemini e poi buttata. Impossibile capire ore dopo o il giorno dopo se la lettura era corretta.

**Soluzione in 3 parti:**

1. **Brain → decision:** Il `contesto_strutturale` viene salvato nel dict decision e include: mappa livelli testuale, posizione nella VA (dentro/sopra/sotto), POC/VAH/VAL weekly, struttura H1, snapshot multi-tf per ogni timeframe, penalità e bonus applicati, timestamp.

2. **Trade Manager → posizione:** Il `contesto_strutturale` viene persistito nel record della posizione aperta (campo aggiunto a `posizioni_aperte[ticker]`).

3. **Alla chiusura → valutazione retrospettiva:** Prima di salvare nello storico, confronta:
   - Struttura H1 all'apertura vs alla chiusura (confermata/invertita?)
   - Avanzamento verso il TP (ha raggiunto il 70%? il 30%?)
   - Nota leggibile: "struttura confermata, trade corretto" / "struttura invertita — lettura errata" / "struttura confermata ma trade perso — timing o SL sbagliato"
   
   Salvato come `valutazione_strutturale` nello storico — disponibile per NightReview e analytics.

### XGBoost — nuove feature e sample weights (chimera_ml.py)
**Problema:** Modello con best_iteration=3 su 24 alberi = pattern troppo debole nei dati di training.
Cause: 3512 trade con SL sbagliati, muri falsi, Gemini cieco. Solo 27 feature, nessuna struttura multi-tf.

**Nuove feature aggiunte (27 → 35):**
- `delta_footprint` — flusso istituzionale granulare
- `struttura_h1_num` — UPTREND=+1, DOWNTREND=-1, allineato alla direzione
- `va_position_num` — posizione nella Value Area settimanale (dentro=0, sopra=+1, sotto=-1)
- `dist_poc_perc` — distanza % dal POC weekly, allineata alla direzione
- `rsi_norm` — RSI normalizzato e orientato nella direzione del trade
- `candlestick_bias_num` — BULLISH=+1, BEARISH=-1, NEUTRO=0
- `tf_1h_allineato` — trend 1h concorde (+1) o contro (-1) la direzione
- `tf_4h_allineato` — trend 4h concorde/contro
- `tf_1d_allineato` — trend 1d concorde/contro

**Sample weights — i trade recenti pesano di più:**
- Trade con contesto strutturale (nuovi): peso 5.0
- Trade post-20/03 (fix SL, muri corretti): peso 3.0
- Trade reali precedenti: peso 1.0
- Trade storici simulati: peso 0.3
Effetto: XGBoost impara principalmente dai dati puliti anche se sono pochi.
Con i nuovi trade (peso 5×) bastano ~30-40 trade strutturati per dominare il dataset.

### Penalità proporzionali e scala voto calibrata (brain_la.py)
**Problema:** Le penalità erano assolute su una scala compressa (Gemini usava quasi sempre voto 7). Una penalità -2 fissa abbatteva qualsiasi voto 7 a 5 (bloccato), indipendentemente dalla qualità del segnale. Il 71% dei segnali reali erano voto 7 → sistema di penalità inefficace.

**Correzione 1 — Cap proporzionale:**
La penalità totale ora è cappata al 40% del voto originale di Gemini.
- Voto 7 → max penalità -2 (7×0.4=2.8 → 2)
- Voto 9 → max penalità -3 (9×0.4=3.6 → 3)
- Evita che penalità eccessive portino voti a numeri assurdi (es. 7-5=2)

**Correzione 2 — Scala voto esplicita nel prompt:**
Aggiunta regola 13 con descrizione dettagliata di ogni voto (4-10):
- 10 = setup perfetto, RARO
- 9 = 4+ confluenze forti
- 8 = 3 confluenze, rischio contenuto
- 7 = 2-3 confluenze, una contro
- 6 = segnale debole
- 5 = FLAT preferibile
- Istruzione esplicita: "Non usare 7 come default. Il 70% dei segnali reali dovrebbe essere 6-8."

**Effetto atteso:** Gemini differenzierà la qualità reale (8-9 per setup forti, 5-6 per deboli). Il sistema penalità/bonus lavorerà su range più ampio e significativo.

### Streak loss — decadimento temporale e no-blocco (brain_la.py + feedback_engine.py)
**Problema:** Lo streak si azzera SOLO al prossimo WIN. Con streak 3-4 e mercato laterale, il bot poteva rimanere bloccato per giorni in un catch-22: non entra perché ha streak alto, non azzeralo streak perché non entra.

**Correzione 1 — Decadimento temporale (feedback_engine.py):**
Lo streak perde 1 punto ogni 24h dall'ultima perdita. 
Streak 3 con ultima perdita 24h fa → streak effettivo 2 → nessun effetto.
Evita blocchi su condizioni di mercato cambiate.

**Correzione 2 — Streak riduce sizing, non voto (brain_la.py):**
- Streak 3 → sizing 90% (prima: voto -1 + sizing -30%)
- Streak 4 → sizing 80%
- Streak 5+ → sizing 70% + voto -1 (solo streak molto alto tocca il voto)
- Cap sizing minimo 60%
Il bot non viene mai completamente bloccato dallo streak — solo ridimensionato.
Questo risolve il catch-22: il bot continua a operare con size ridotta finché non vince.

### XGBoost — gate qualità AUC, modalità osservazione passiva (chimera_ml.py)
**Problema:** AUC 0.425 con best_iteration=0. Il modello era peggio del random e modificava comunque le decisioni (in teoria — in pratica la confidenza 0.103 lo bloccava già, ma il rischio era reale).

**Correzione:** Gate di qualità doppio in correggi_voto():
1. AUC >= 0.52: solo sopra questa soglia il modello interviene sulle decisioni
2. Confidenza >= 0.65: soglia esistente mantenuta

Sotto AUC 0.52 → modalità osservazione passiva: predice, salva prob nel snapshot, NON modifica il voto. Il modello continua ad accumularsi dati per migliorare senza fare danni.

**stato() aggiornato:** Espone ora 'auc', 'best_iteration', 'ml_attivo_decisioni' e 'motivo' per monitoraggio via /ml su Telegram.

**Quando si attiverà:** Dopo completamento enrich_storico.py (struttura_h1 e RSI su >50% dei trade) + riaddestramente → AUC dovrebbe salire sopra 0.52 e il modello si attiverà automaticamente.

### Fix NameError sl_f not defined (brain_la.py)
**Problema:** `sl_f` e `tp_f` usati a riga 1880 ma non definiti — la chiamata a `determina_tp_sl_ts` era andata persa quando ho aggiunto il blocco `contesto_strutturale` in mezzo al flusso.
**Correzione:** Reinserita la chiamata `tp_f, sl_f, _ = self.determina_tp_sl_ts(...)` dentro il blocco `if direzione_ia != "FLAT"` prima dell'assegnazione a `decision`.

### Fix EOrder:Invalid leverage su chiusura posizione (performer_la.py)
**Problema:** Chiusura BTC falliva con "EOrder:Invalid leverage". Il bot passava la leva esplicita nell'ordine di chiusura (reduce_only). Se la leva in memoria (es. 5x) non corrisponde a quella reale su Kraken (es. 4x per disallineamento), Kraken rifiuta l'ordine.
**Correzione:** Per ordini reduce_only (chiusura) non viene più passata la leva esplicita. Kraken la conosce già dalla posizione aperta. Solo `reduce_only: True` viene inviato per le posizioni margin.

### Fix PnL sballato rispetto a Kraken (performer_la.py + trade_manager.py)
**Problema:** Bot mostrava +0.20$ (1.14%) su BTC LONG mentre Kraken mostrava +2.90$ (1.07%). Il PnL era calcolato internamente con size/prezzo sbagliati.

**Causa 1:** `get_trade_pnl_real` usava `info.get('net', 0)` che in certi casi restituisce il valore in valuta base (BTC) non in USD. Fix: controlla esplicitamente `posstatus == 'closed'` e gestisce i casi dove `net` non è disponibile.

**Causa 2:** `pnl_netto_perc` divideva per `p_entrata * size / leverage` (il margine) invece che per `p_entrata * size` (la posizione). Con leva 10x questo moltiplicava la % per 10 → 1.14% diventava 11.42%. Fix: usa la posizione totale come Kraken, non il margine.

### Sistema penalità/bonus → correzione proporzionale (brain_la.py)
**Problema:** Le penalità erano veti mascherati. BTC corr -2 su voto 7 con cap 40% = max -2 → occupava l'intero margine disponibile, bloccando il trade indipendentemente dagli altri segnali. Non era "uno strumento tra tanti" ma un override.

**Principio nuovo:** Ogni strumento contribuisce proporzionalmente. L'unico veto è la soglia di ingresso (voto < 6).

**Correzione extra [-3, +3] calcolata da:**
1. BTC correlazione: peso proporzionale alla corr reale (corr 0.73 → peso 0.73×0.8), max ±0.8
2. Multi-timeframe (1d 40%, 4h 40%, 1h 20%): media pesata dei TF trending, max ±0.7
3. Order Flow (OFI + Whale delta): i più affidabili, CVD come moltiplicatore, max ±1.0
4. Struttura H1 (BoS/CHoCH): max ±0.5
5. Candlestick bias: +0.4 normale, +0.8 pattern forti (ENGULFING, MARUBOZU, PIN_BAR)

**Risultato calibrazione su 24 marzo:** 6/7 trade corretti (86%).

### Fix EGeneral:Invalid arguments:reduce_only (performer_la.py + trade_manager.py)
**Problema:** TP e SL fallivano con "reduce_only only valid for leveraged orders". 
**Causa:** `private_post_addorder` di Kraken non supporta il parametro `reduce_only`. 
Solo l'API REST v3 lo supporta, non la v2 usata da CCXT.
**Correzione:** Rimosso `reduce_only` da tutti gli ordini di protezione (TP, SL, chiusura totale). 
Kraken identifica automaticamente un ordine come chiusura quando è sul lato opposto alla posizione aperta con la stessa leva.

### Fix EOrder:Invalid leverage su chiusura totale (trade_manager.py)
**Problema:** _esegui_chiusura_totale falliva con "EOrder:Invalid leverage" perché passava la leva dalla memoria del bot (es. 10x) che non corrispondeva alla leva reale su Kraken.
**Correzione:** Rimossa completamente la leva dall'ordine di chiusura market. Kraken conosce già la leva dalla posizione aperta — non serve specificarla per chiudere.

### Fix EOrder:Insufficient funds su chiusura posizione margin (trade_manager.py)
**Problema:** Chiusura posizioni margin falliva con "EOrder:Insufficient funds" perché il bot inviava un ordine market normale sul lato opposto — Kraken lo interpretava come apertura di nuova posizione contraria, richiedendo margine libero.
**Correzione:** Usato `ordertype=settle-position` via `private_post_addorder`. Questo chiude direttamente la posizione margin esistente senza richiedere margine aggiuntivo — è il metodo corretto per chiudere posizioni margin su Kraken.

### Sincronizzazione Kraken → Bot come specchio (trade_manager.py)
**Problema:** Il bot adottava posizioni orfane silenziosamente senza alertare. Size e leva non venivano aggiornate dai dati reali Kraken. Il bot non era lo specchio di Kraken.

**Refactor sincronizza_con_exchange:**
- Kraken è la fonte di verità assoluta
- Ogni ciclo: size e leva vengono aggiornate dai dati reali Kraken
- Posizioni interne non più su Kraken vengono chiuse automaticamente
- Posizioni orfane (su Kraken ma non nel DB): alert Telegram immediato con dettagli, adozione con SL emergenza
- Log chiaro di ogni aggiornamento (ticker, size, leva)

### Kraken WS v2 executions — specchio real-time posizioni (ws_manager.py + bot_la.py)
**Problema:** Il bot scopriva le chiusure/aperture solo al ciclo REST successivo (ogni ~60s). Causava desync tra bot e Kraken, posizioni orfane, errori di leva.

**Soluzione:** Aggiunto canale privato `executions` Kraken WebSocket v2 in ws_manager.py.
- Ogni esecuzione ordine (apertura/chiusura/modifica) notifica il bot in real-time via WebSocket
- Callback → trade_manager.sincronizza_con_exchange() chiamato istantaneamente
- posizioni_mirror e ordini_mirror aggiornati in memoria senza chiamate REST
- get_posizione_mirror(asset) e get_ordine_mirror(order_id) disponibili

**Attivazione:** ws_mgr.set_credentials(key, secret) chiamato in bot_la.py dopo init engine.
**Dipendenza:** pip install websockets (se non già presente)

### Rimosso sistema proporzionale post-Gemini (brain_la.py)
**Motivo:** Gemini riceve già tutti i dati (OFI, Whale, BTC corr, MTF, H1, Candele, Z-Score) nel JSON e nelle note esplicative del prompt. Le correzioni post-Gemini erano ridondanti e a volte contraddicevano il ragionamento di Gemini.
**Rimosso:** ~134 righe del sistema di correzione proporzionale (BTC corr, MTF, OFI+Whale, H1, Candele, Z-Score come extra [-3,+3]).
**Mantenuto:** Policy adjust (streak sizing, alpha decay sizing), soglia di ingresso come unico veto.
**Risultato:** Gemini è l'unico decisore sul voto. I dati sono nel prompt, lui li pesa nel ragionamento.

### Specchio Kraken all'avvio — _sincronizza_da_kraken_avvio (trade_manager.py)
**Problema:** All'avvio il bot caricava il DB (potenzialmente vuoto) e non sapeva delle posizioni aperte su Kraken. Posizioni aperte manualmente o durante un crash venivano ignorate.
**Fix:** Aggiunto _sincronizza_da_kraken_avvio() chiamato nell'__init__ prima di _ricalcola_pnl_odierno.
- Legge posizioni reali da Kraken (force=True)
- Posizioni su Kraken non nel DB → adottate automaticamente con SL/TP emergenza + alert Telegram
- Posizioni nel DB non su Kraken → rimosse
- Posizioni in entrambi → size e leva aggiornate dai dati reali Kraken
**Risultato:** Il bot parte sempre dallo stato reale di Kraken, indipendentemente dal DB.

### Analisi continua BTC/ETH + ghost positions + sentinella (bot_la.py, trade_manager.py, engine_la.py)
**Problema 1:** BTC e ETH dopo il primo ciclo sparivano — la sentinella non triggerava e il quarto d'ora era troppo raro.
**Fix:** Aggiunta analisi forzata ogni 5 minuti (_forza_periodico) indipendentemente dalla sentinella e dai quarti d'ora.

**Problema 2:** Posizioni ghost (adottate da Kraken senza analisi) non alimentavano XGBoost alla chiusura.
**Fix:** _chiudi_statisticamente notifica chimera_ml.registra_trade() con voto stimato=5 e fonte='GHOST_KRAKEN'.

**Problema 3:** Sentinella troppo insensibile (0.3%) perdeva movimenti importanti.
**Fix:** Soglia abbassata a 0.15% — la sentinella triggera il doppio delle volte.

### Trade virtuali per XGBoost (feedback_engine.py, bot_la.py)
**Problema:** Le analisi scartate (voto < soglia) venivano perse — XGBoost non imparava da esse.
**Fix:** 
- `registra_analisi_scartata` ora filtra voto >= 5 e direzione non FLAT, salva snapshot completo (23 campi) con SL/TP di Brain
- `verifica_esiti_ghost` monitora il prezzo ad ogni ciclo finché SL o TP viene colpito (non più timeout 1h)
- Quando chiuso → passa automaticamente a ChimeraML con fonte=VIRTUAL_BRAIN
- Timeout 24h per ghost non risolti
- Zero impatto su PnL reale — solo addestramento
- bot_la.py passa sl e tp a registra_analisi_scartata