        ...
        def index():
            ...
            for asset, p in posizioni.items():
                try:
                    ...
                    _err.capture(e, sys._getframe().f_code.co_name, {"module": "Dashboard"})
                    print(f"Errore calcolo P&L Dashboard per {asset}: {e}")
                    p['pnl_perc'] = 0.0
            # ------------------------------------------------

            # 2. Calcolo Win Rate Reale dallo storico (FIX: usa db_manager invece del JSON non più esistente)
            win_rate = "0.0%"
            try:
                from core.database_manager import db_manager
                storico = db_manager.get_storico()
                if storico:
                    vinti = len([t for t in storico if float(t.get('pnl_netto_usd', t.get('pnl_finale', 0))) > 0])
