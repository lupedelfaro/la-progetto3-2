...
class FeedbackEngine:
    ...
    def get_feedback_summary(self, asset=None):
        """
        Genera un report qualitativo per l'IA basato sugli ultimi 30 giorni.
        Trasforma i dati numerici in lezioni testuali comprensibili.
        """
        if not isinstance(self.cache, list):
            self.logger.error("⚠️ Cache feedback non è una lista, la resetto.")
            self.cache = []
        ...
        # 1. Filtro Temporale: prendiamo solo gli ultimi 30 giorni
        feedback_recenti = []
        for r in self.cache:
            try:
                # Se l'asset è specificato, filtriamo anche per quello
                if asset and r.get("asset") != asset:
                    continue
                
                dt_evento = datetime.fromisoformat(r.get("timestamp"))
                if (ora - dt_evento).days <= trenta_giorni_fa:
                    feedback_recenti.append(r)
            except Exception:
                continue

        # Prendiamo solo gli ultimi 15 eventi per non confondere l'IA
        data = feedback_recenti[-15:]
        
        if not data:
            return "Nessuna lezione identificata negli ultimi 30 giorni. Procedi con cautela."

        wins = sum(1 for r in data if r.get("outcome") == "WIN")
        total = len(data)
        win_rate = (wins / total * 100) if total > 0 else 0

        # 2. Costruzione del report "parlante" per Gemini
        report = f"--- MEMORIA OPERATIVA (Ultimi 30gg) ---\n"
        report += f"Win Rate: {win_rate:.1f}% su {total} eventi.\n\n"
        report += "LEZIONI DAI TRADE PRECEDENTI:\n"

        for r in data:
            esito = "✅ WIN" if r.get("outcome") == "WIN" else "❌ LOSS"
            voto = r.get("score", "?")
            motivo = r.get("motivi", "N/A")
            # Estraiamo le info di mercato se presenti
            snap = r.get("snapshot_mercato", {})
            z = snap.get("z_score", "N/A") if snap else "N/A"
            
            report += f"- Voto {voto} | {esito}: {motivo} (Z-Score: {z})\n"
